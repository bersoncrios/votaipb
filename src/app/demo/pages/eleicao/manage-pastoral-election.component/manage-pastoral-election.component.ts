...
@Component({
    selector: 'app-manage-pastoral-election',
    standalone: true,
    imports: [
        CommonModule,
        RouterModule,
    ...
    templateUrl: './manage-pastoral-election.component.html',
    styleUrls: ['./manage-pastoral-election.component.scss']
})
export class ManagePastoralElectionComponent implements OnInit {
    ...
    getCargoTitulo(eleicao: EleicaoPastoral, cargoId: string): string {
        const cargo = eleicao.cargos?.find(c => c.id === cargoId);
        return cargo ? cargo.titulo : 'Cargo Pastoral em Votação';
    }

    async onAbrirVotacao(eleicao: EleicaoPastoral, cargo: CargoPastoral) {
    ...
    async onEncerrarVotacao(eleicao: EleicaoPastoral, cargo: CargoPastoral) {
    ...
    async onCopiarLink(eleicaoId: string) {
        ...
        const link = `${origin}/votar-pastor/${eleicaoId}`;
        try {
