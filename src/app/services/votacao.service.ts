...
export class VotacaoService {
  ...
  async getCedulaAberta(eleicaoId: string): Promise<CedulaAberta | null> {
    ...
    const escrutinio = cargo.escrutinios.find((e) => e.numero === escrutinioNum && e.status === 'aberto');
    if (!escrutinio) {
      return null;
    }
  ...
  async registrarVoto(eleicaoId: string, cargoId: string, escrutinioNum: number, eleitorId: string, candidatoId: string): Promise<void> {
    ...
    try {
      await runTransaction(this.db, async (transaction) => {
        ...
        const escrutinioIndex = novosCargos[cargoIndex].escrutinios.findIndex((e) => e.numero === escrutinioNum && e.status === 'aberto');
        ...
        const jaVotou = escrutinioAlvo.votos.some((v: Voto) => v.eleitorId === eleitorId);
